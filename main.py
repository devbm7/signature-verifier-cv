...
def capture_image_from_cam_into_temp(sign=1):
    ...
    while True:
        ...
        if k % 256 == 27:
            # ESC pressed
            print("Image successfully grabed")
            break
        elif k % 256 == 32:
            # SPACE pressed
...
root = tk.Tk()
root.title("Signature Matching Project")
root.geometry("500x700") 
x_label = tk.Label(root, text="Compare Two Signatures:",bg='yellow',fg='red',font=("Comicsans,15,bold"),borderwidth=15)
x_label.pack()

img1_message2 = tk.Label(root, text="Signature 1", font=10)
img1_message2.place(x=10, y=120)

image1_path_entry = tk.Entry(root, font=10)
image1_path_entry.place(x=150, y=120)

img1_capture_button = tk.Button(root, text="Capture", font=10, command=lambda: captureImage(ent=image1_path_entry, sign=1))
...
img2_capture_button = tk.Button(root, text="Capture", font=10, command=lambda: captureImage(ent=image2_path_entry, sign=2))
